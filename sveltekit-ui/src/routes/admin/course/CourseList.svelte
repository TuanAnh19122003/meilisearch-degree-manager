<script lang="ts">
	...
	$: totalPages = Math.ceil((pagination?.total || 0) / (pagination?.pageSize || 1));
		...
		<table class="w-full border-collapse text-sm">
			<thead class="bg-gray-50 text-gray-700">
				<tr>
					...
					<th class="px-4 py-3 text-left font-medium">Mã môn học</th>
					<th class="px-4 py-3 text-left font-medium">Tên môn học</th>
					<th class="px-4 py-3 text-left font-medium">Tín chỉ</th>
			...
			<tbody class="divide-y divide-gray-100">
				{#each data as item, index}
					<tr class="transition-colors hover:bg-gray-50">
						...
						<td class="px-4 py-3 text-center">{item.credit}</td>
		...
		{#each data as item}
			...
			>
				<div class="flex-1">
					...
					<p class="text-sm text-gray-500">Tín chỉ: {item.credit}</p>
				</div>
...
<div class="mt-6 flex items-center justify-between text-sm">
	<span class="text-gray-600">
		Trang {pagination.current} / {totalPages > 0 ? totalPages : 1}
	</span>

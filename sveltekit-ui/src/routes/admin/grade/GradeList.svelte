		...
		<table class="w-full border-collapse text-sm">
			<thead class="bg-gray-50 text-gray-700">
				<tr>
					...
					<th class="px-4 py-3 text-left font-medium">Sinh viên</th>
					<th class="px-4 py-3 text-left font-medium">Môn học</th>
					<th class="px-4 py-3 text-left font-medium">Điểm</th>
			...
			<tbody class="divide-y divide-gray-100">
				{#each data as item, index}
					<tr class="transition-colors hover:bg-gray-50">
						...
						<td class="px-4 py-3">{item.course?.name}</td>
						<td class="px-4 py-3 font-mono text-blue-600">{item.grade}</td>
...
{#if openMenuId && currentItem}
	...
	>
		...
		>
			<Pencil class="h-4 w-4 text-gray-600" /> Sửa
		</button>
		<button
			class="flex w-full items-center gap-2 px-4 py-2 text-left text-red-600 hover:bg-gray-50"
